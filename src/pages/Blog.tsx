...
import { BLOG_POST_IMAGES } from '../constants';
...
export default function Blog() {
  ...
  const blog = t<{
    ...
    posts: Array<{ id: string; title: string; excerpt: string; author: string; date: string; category: string }>;
  }>('blog');
    ...
    <div className="pt-32 pb-20">
      <header className="text-center mb-20 px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <span className="text-gold uppercase tracking-[0.4em] text-xs font-bold mb-4 block">{blog.eyebrow}</span>
          <h1 className="text-5xl md:text-7xl font-serif font-bold mb-8">{blog.title}</h1>
          <p className="text-white/50 max-w-2xl mx-auto font-light">{blog.description}</p>
        </motion.div>
      </header>
        ...
        {blog.posts.map((post, index) => (
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <p className="text-white/60 font-light leading-relaxed line-clamp-3">{post.excerpt}</p>
              <button className="flex items-center gap-2 text-gold uppercase tracking-widest text-xs font-bold pt-4">
                {blog.readMore} <ArrowRight size={14} className="group-hover:translate-x-2 transition-transform" />
              </button>
            </div>
          </motion.article>
        ))}
      ...
      <section className="mt-32 max-w-4xl mx-auto px-6">
        <div className="glass p-12 rounded-3xl text-center space-y-8">
          <h2 className="text-3xl font-serif font-bold">{blog.newsletter.title}</h2>
          <p className="text-white/50 font-light">{blog.newsletter.description}</p>
